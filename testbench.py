...
async def generate_clock(dut: HierarchyObject) -> None:
    while True:
        dut.CLK.value = 0
...
async def test_module(dut: HierarchyObject) -> None:
    ...
    op_value = {x: 0 for x in range(0, 16)}

    for _ in range(1000):
        x = random.randint(0, 15)
        dut.ACT.value = random.randint(0, 1)
        dut.OP.value = x
        dut.MOVI.value = random.randint(0, 3)
        dut.REG_A.value = random.randint(0, 2**32 - 1)
        dut.REG_B.value = random.randint(0, 2**32 - 1)
        dut.MEM.value = random.randint(0, 2**32 - 1)
        dut.IMM.value = random.randint(0, 2**32 - 1)
    ...
    (Path(__file__).resolve().parent / 'data.txt').write_text(json.dumps(op_value))
